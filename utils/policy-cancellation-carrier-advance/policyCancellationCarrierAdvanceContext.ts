export type PolicyCancellationCarrierAdvanceContext = {
  policyNumber: string;
  timestamp: number;
...
let _ctx: PolicyCancellationCarrierAdvanceContext = {
  policyNumber: '',
  timestamp: 0,
  preparedFilePath: '',
  preparedFileName: '',
...
export function setChargebackAmount(value: number): void { _ctx.chargebackAmount = value; }
export function getChargebackAmount(): number {
  if (!_ctx.chargebackAmount) throw new Error('Chargeback amount not set — run chargeback statement preparation first');
  return _ctx.chargebackAmount;
}

export function clearPolicyCancellationCarrierAdvanceContext(): void {
  _ctx = {
    ...
    advanceAmount: 0, arfValue: 0, agencyCredit: 0, carrierName: '',
    commissionRecoveryCount: 0, pendingPayment: 0, chargebackAmount: 0,
  };
}
