...
import {
  ADVANCE_ONLY,
  advanceOnlyTemplatePath,
  advanceOnlyRecoveryTemplatePath,
} from '../../test-data/advance-only/validateAdvanceOnly';
import { registerGeneratedFile } from '../generatedFileCleanup';

export type AdvanceOnlyPreparedFile = {
  absolutePath: string;
  fileName: string;
  carrierName: string;
  customerUid: string;
...
/**
 * Read the advance-only Excel template, increment the Customer UID by +1
 * on every data row, extract agent ID and product name alias, then save
 * a timestamped copy for upload.
 */
export async function prepareAdvanceOnlyStatementFile(): Promise<AdvanceOnlyPreparedFile> {
  const templatePath = advanceOnlyTemplatePath();
  if (!fs.existsSync(templatePath)) {
    throw new Error(`Advance-only template not found: ${templatePath}`);
  }

  fs.mkdirSync(ADVANCE_ONLY.generatedDir, { recursive: true });

  const wb = new ExcelJS.Workbook();
  await wb.xlsx.readFile(templatePath);
  const sheet = wb.worksheets[0];
  if (!sheet) throw new Error('Advance-only template workbook has no worksheets');
  ...
  if (!uidCol) {
    throw new Error(`Column "Customer UID" not found in advance-only template: ${templatePath}`);
  }
  ...
  for (let rowIndex = 2; rowIndex <= sheet.rowCount; rowIndex++) {
    ...
    // e.g. "MY-CUST-007" → "MY-CUST-008", "ATENA-ADV-TEST-A007" → "ATENA-ADV-TEST-A008"
    ...
    if (currentUid) {
      ...
      if (numMatch) {
        ...
        uidCell.value = `${ADVANCE_ONLY.customerUidPrefix}1`;
      }
    }
  ...
  const fileName = `AdvanceOnly-${stamp}.xlsx`;
  const absolutePath = path.join(ADVANCE_ONLY.generatedDir, fileName);
  ...
  return {
    absolutePath,
    fileName,
    carrierName: ADVANCE_ONLY.carrierName,
    customerUid,
...
/**
 * Read the advance-only recovery template, replace the Customer UID with
 * the provided policy number, and save a timestamped copy for upload.
 */
export async function prepareAdvanceOnlyRecoveryFile(
  policyNumber: string,
  timestamp: number,
): Promise<AdvanceOnlyPreparedFile> {
  const templatePath = advanceOnlyRecoveryTemplatePath();
  if (!fs.existsSync(templatePath)) {
    throw new Error(`Advance-only recovery template not found: ${templatePath}`);
  }
  ...
  await wb.xlsx.readFile(templatePath);
  const sheet = wb.worksheets[0];
  if (!sheet) throw new Error('Advance-only recovery template workbook has no worksheets');
  ...
  if (!uidCol) {
    throw new Error(
      `Column "Customer UID" not found in advance-only recovery template: ${templatePath}`,
    );
  }
  ...
  // Save timestamped copy
  const fileName = `AdvanceOnly-Recovery-${timestamp}.xlsx`;
  const absolutePath = path.join(ADVANCE_ONLY.generatedDir, fileName);
  await wb.xlsx.writeFile(absolutePath);
  registerGeneratedFile(absolutePath);
