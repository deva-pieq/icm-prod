...
import {
  COMMISSION_ONLY,
  commissionOnlyTemplatePath,
  commissionOnlyRecoveryTemplatePath,
} from '../../test-data/advance-only/validateCommissionOnly';

export type CommissionOnlyPreparedFile = {
  absolutePath: string;
  fileName: string;
  carrierName: string;
  customerUid: string;
...
/**
 * Read the commission-only Excel template, increment the Customer UID by +1
 * on every data row, extract agent ID and product name alias, then save
 * a timestamped copy for upload.
 */
export async function prepareCommissionOnlyStatementFile(): Promise<CommissionOnlyPreparedFile> {
  const templatePath = commissionOnlyTemplatePath();
  if (!fs.existsSync(templatePath)) {
    throw new Error(`Commission-only template not found: ${templatePath}`);
  }

  fs.mkdirSync(COMMISSION_ONLY.generatedDir, { recursive: true });

  const wb = new ExcelJS.Workbook();
  await wb.xlsx.readFile(templatePath);
  const sheet = wb.worksheets[0];
  if (!sheet) throw new Error('Commission-only template workbook has no worksheets');
  ...
  const productNameCol =
    ...
    columns.get('productnamealias') ??
    columns.get('product name') ??
    columns.get('scale name/adjustment description') ??
    columns.get('scalenamedescription') ??
    columns.get('scale name');

  if (!uidCol) {
    throw new Error(
      `Column "Customer UID" not found in commission-only template: ${templatePath}`,
    );
  }
  ...
  for (let rowIndex = 2; rowIndex <= sheet.rowCount; rowIndex++) {
    ...
    if (currentUid) {
      ...
      if (numMatch) {
        ...
        uidCell.value = `${COMMISSION_ONLY.customerUidPrefix}1`;
      }
    }
  ...
  const fileName = `CommissionOnly-${stamp}.xlsx`;
  const absolutePath = path.join(COMMISSION_ONLY.generatedDir, fileName);
  ...
  return {
    absolutePath,
    fileName,
    carrierName: COMMISSION_ONLY.carrierName,
    customerUid,
...
/**
 * Read the commission-only recovery template, replace the Customer UID with
 * the provided policy number, and save a timestamped copy for upload.
 */
export async function prepareCommissionOnlyRecoveryFile(
  policyNumber: string,
  timestamp: number,
): Promise<CommissionOnlyPreparedFile> {
  const templatePath = commissionOnlyRecoveryTemplatePath();
  if (!fs.existsSync(templatePath)) {
    throw new Error(`Commission-only recovery template not found: ${templatePath}`);
  }
  ...
  await wb.xlsx.readFile(templatePath);
  const sheet = wb.worksheets[0];
  if (!sheet) throw new Error('Commission-only recovery template workbook has no worksheets');
  ...
  if (!uidCol) {
    throw new Error(
      `Column "Customer UID" not found in commission-only recovery template: ${templatePath}`,
    );
  }
  ...
  // Save timestamped copy
  const fileName = `CommissionOnly-Recovery-${timestamp}.xlsx`;
  const absolutePath = path.join(COMMISSION_ONLY.generatedDir, fileName);
  await wb.xlsx.writeFile(absolutePath);
