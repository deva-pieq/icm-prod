...
Given(
  'a commission report statement file is prepared from the inbox',
...
When('I open the commission report upload page', async ({ commissionReportValidationPage }) => {
  await commissionReportValidationPage.openUploadPage();
});

When(
  'I upload the prepared commission report statement file',
  async ({ commissionReportValidationPage }) => {
...
When(
  'I pick the statement type from the prepared commission report file',
  async ({ commissionReportValidationPage }) => {
...
When(
  'I submit the upload for processing in commission report validation',
  async ({ commissionReportValidationPage }) => {
...
Then(
  'the uploaded file appears in the recently uploaded grid in commission report validation',
  async ({ commissionReportValidationPage }) => {
...
Then(
  'extract processing completes and the file ID is captured in commission report validation',
  async ({ commissionReportValidationPage }) => {
...
Then(
  'the upload row shows status {string} and stage {string} in commission report validation',
...
When(
  'I open the review page for the uploaded file in commission report validation',
  async ({ commissionReportValidationPage }) => {
...
Then(
  'the review page URL contains {string} in commission report validation',
...
Then(
  'the review page heading is {string} in commission report validation',
...
When(
  'I capture review lines and policy Agent commission values in commission report validation',
  async ({ commissionReportValidationPage }) => {
    await commissionReportValidationPage.captureReviewAndPolicyCommissionData();
  },
);

Then(
  'the commission report CSV skeleton is written with expected amounts in commission report validation',
  async () => {
...
When(
  'I complete the review and confirm in commission report validation',
  async ({ commissionReportValidationPage }) => {
...
Then(
  'a success toast notification appears in commission report validation',
  async ({ commissionReportValidationPage }) => {
...
When(
  'I wait for 3 seconds and refresh the data grid in commission report validation',
  async ({ commissionReportValidationPage }) => {
...
Then(
  'the upload stage changes to {string} in commission report validation',
...
When(
  'I open view commission report from the upload kebab in commission report validation',
  async ({ commissionReportValidationPage }) => {
...
When(
  'I fill Agent Split Amount from the commission report into the CSV in commission report validation',
  async ({ commissionReportValidationPage }) => {
    await commissionReportValidationPage.fillCommissionFromReportIntoCsv();
  },
);

Then(
  'calculated value times members equals commission from report for every line in commission report validation',
  async ({ commissionReportValidationPage }) => {
    await commissionReportValidationPage.writeAndAssertCommissionReportCsv();
  },
);

Then('the commission report CSV file is finalized for this statement run', async () => {
  const prepared = getCommissionReportPreparedFile();
  expect(fs.existsSync(prepared.reportSheetPath)).toBeTruthy();
  const content = fs.readFileSync(prepared.reportSheetPath, 'utf8');
